...
| `match_key` | `string` | none | Identifier field to match against the user's destination list (e.g. `"destination"`). Must be a single bare identifier name (no whitespace, `/` or NUL) and must be present in the schema's `identifier` with `required: true`. It cannot select a `PointCloudHandler`, because watch/replay subscribers provide a polygon and Aviso replaces `point_cloud` with wildcard routing before plugin authorization. When a topic is configured, the field must also appear in `topic.key_order` so delivery is filtered by the authorized value. Schemas without a topic retain generic routing behavior. |
