...
| 403 storm on watch/replay | `aviso_ecpds_access_decisions_total{outcome="deny_destination"}` rate | `event_name=auth.ecpds.check.denied` with `reason=destination_not_in_user_list` | ECPDS revoked destinations for one or more users; or a client suddenly started passing the wrong `destination`. Cross-check with the same users hitting the ECPDS web UI directly. |
| 403 with `reason=match_key_missing` | same | `event_name=auth.ecpds.check.denied` with `reason=match_key_missing` | The schema's `match_key` field is required, but a client is omitting it. Validation should have prevented this — investigate config drift. |
...
| `auth.ecpds.check.denied` | warn | `enforce_ecpds_auth` | `event_type`, `username`, `reason` ∈ {`DestinationNotInList`, `MatchKeyMissing`} |
| `auth.ecpds.check.unavailable` | warn | `enforce_ecpds_auth` | `event_type`, `username`, `fetch_outcome` |
...
| `auth.ecpds.fetch.divergence` | warn | `EcpdsClient` (merge) | `server_index` (strict) or `pairwise_divergence_count` + `reachable_servers` (any_success), `divergence_count` |
