...
- **Health and metrics.** `GET /health` returns `200 OK` for probes. `/metrics` runs on a separate port (configured under `metrics:`) and exposes Prometheus text-format counters for notifications, SSE connections, auth outcomes, and (when built with `--features ecpds`) ECPDS cache and access decisions.
